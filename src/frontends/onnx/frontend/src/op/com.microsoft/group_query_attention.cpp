...
ov::OutputVector group_query_attention(const ov::frontend::onnx::Node& node) {
    ...
    const int64_t head_size = nodes[0].get_partial_shape()[2].get_length() / (num_heads + kv_num_heads * 2);
    const auto head_size_node = v0::Constant::create(ov::element::i64, ov::Shape{1}, {head_size});
    ...
    auto construct_kv_cache = [&](const ov::Output<ov::Node>& past, const ov::Output<ov::Node>& current) {
        ...
        return std::make_shared<v0::Concat>(ov::OutputVector{past_datas, current}, 2);
    };
    ...
    // std::shared_ptr<ov::Node> alpha = nullptr;
    // if (scale != 0.0f) {
    //     alpha = v0::Constant::create(ov::element::f32, ov::Shape{}, {scale});
    // }

    // std::shared_ptr<ov::Node> output = std::make_shared<v13::ScaledDotProductAttention>(Q->output(0),
    //                                                                      K->output(0),
    //                                                                      V->output(0),
